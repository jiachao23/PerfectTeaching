layui.define(['laypage', 'layer','table','common','util','form'], function (exports) {
    ...
    table.on('tool(table)', function(obj){
        ...
        if(obj.event === 'del'){
            ...
            common.frame_show('分类编辑','/admin/category/form?id='+data.id,'720','430');
        }
    });
    //监听状态
    form.on('checkbox(status)', function(data){
        $.ajax({
            ...
            dataType: "json",
            data: {"type":"status"},
            url: "/admin/category/" + data.value + "/change",
    ...
    $('#addCategory').click(function () {
        ...
        setTimeout(function () {
            layer.close(index);
            common.frame_show('分类添加','/admin/category/form','720','430');
            // layer.msg('打开添加窗口');
        }, 500);
    });
    ...
    var datalist = {
        ...
        editData: function (id) {
            common.frame_show('分类编辑','/admin/category/form?id='+id,'720','430');
        }
    };
    function del(id) {
