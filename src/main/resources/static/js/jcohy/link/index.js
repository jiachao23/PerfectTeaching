layui.define(['laypage', 'layer',  'table','common','form','util'], function (exports) {
    ...
    table.render({
        ...
        ,cols: [[ //表头
            ...
            ,{field:'status', title: '是否可见', templet: '#choicesTpl'}
    ...
    table.on('tool(table)', function(obj){
        ...
        if(obj.event === 'del'){
            ...
            common.frame_show('分类编辑','/admin/link/form?id='+data.id);
        }
    });
    //分页
    laypage.render({
    ...
    form.on('checkbox(status)', function(data){
        $.ajax({
            ...
            dataType: "json",
            data: {"type":"status"},
            url: "/admin/link/" + data.value + "/change",
    ...
    $('#addLink').click(function () {
        ...
        setTimeout(function () {
            layer.close(index);
            common.frame_show('分类添加','/admin/link/form');
            // layer.msg('打开添加窗口');
        }, 500);
    });
    ...
    var datalist = {
        ...
        editData: function (id) {
            common.frame_show('分类编辑','/admin/link/form?id='+id);
        }
    };
    function del(id) {
